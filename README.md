...
This will run all Coverity Analysis phases by using the Coverity build capture (cov-build). This action is using [lejouni/coverity-commit-checker](https://github.com/lejouni/coverity-commit-checker) to check that is the actual Coverity commit needed or not. If there are introduced any new findings or fixed any existing ones, then commit will be done otherwise no commits.
...
NOTE: Java based repositories can be also analyzed with cov-build capture and you can use this action to do that.
...
| build_command | Application build command. Ex. mvn clean install | - | true |
| cov_build_params | Additional parameters for cov-build phase| --fs-capture-search ${{github.workspace}} --fs-capture-search-exclude-regex \"[/\\\\]test[/\\\\]\" | false |
...
## Usage examples
Run the Coverity full build analysis.
```yaml
...
```
Run the Coverity incremental build analysis with project mode.
```yaml
